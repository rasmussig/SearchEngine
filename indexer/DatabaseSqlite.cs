...
namespace Indexer
{
    public class DatabaseSqlite : IDatabase
    {
        ...
        // Method not in use at the moment
        ...
        public Dictionary<string, int> GetWordFrequencies()
        {
            ...
            var selectCmd = _connection.CreateCommand();
            selectCmd.CommandText = "SELECT word.name, COUNT(*) as freq FROM occ JOIN word ON occ.wordId = word.id GROUP BY word.name ORDER BY freq DESC";
