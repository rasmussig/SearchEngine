    ...
    public class DatabaseSqlite : IDatabase
    {
        ...
        public List<int> GetWordIds(string[] query, out List<string> outIgnored, bool caseSensitive = false)
        {
            ...
            foreach (var aWord in query)
            {
                ...
                {
                    // Case insensitive - find all case variations
                    foreach (var wordEntry in mWords)
                    {
                        if (string.Equals(wordEntry.Key, aWord, StringComparison.OrdinalIgnoreCase))
                        {
                            res.Add(wordEntry.Value);
                            found = true;
                        }
                    }
                }
